...
/** Global ScrollSmoother instance */
let smoother = null;
...
function initScrollSmoother() {
  ...
  smoother = ScrollSmoother.create({
    ...
    smooth: prefersReduce ? 0 : 1.5,
    effects: !prefersReduce,
    smoothTouch: 0.1,
  });
...
function initFloatingNav() {
  ...
  function updateActiveFromScroll() {
    const probeY = getScrollY() + window.innerHeight * 0.25;
    let activeId = 'hero';
  ...
  // Also hook into ScrollTrigger refresh for accuracy
  if (typeof ScrollTrigger !== 'undefined') {
    ScrollTrigger.addEventListener('refresh', onScrollOrResize);
  }
